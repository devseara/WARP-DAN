...
def open_cards(p=None):
    obj=m.ready(p if p is not None else snapshot(flags=2|4,level=0));m.click(obj,217,127)
...
for i,cost in enumerate((500000,1200000,2500000,5000000,9000000)):
    ...
    pix,w,h=m.pixels(q)
    assert m.r(pix+((25+i*48)*w+6)*4)==0xFFACBFE3,'Approved blue bevel'
    assert m.r(pix+((70+i*48)*w+190)*4)==0xFFF4FCFF,'Gap below card must match icy popup body'
    assert m.r(pix+((27+i*48)*w+190)*4)==0xFFF4FAFF,'Inset pale-blue panel'
    assert any((m.r(pix+((33+i*48+y)*w+16+x)*4)&255)<180 for y in range(28) for x in range(28)),'Missing gold crown'
before=len(m.sent);m.click(q,170,45);m.preview(q,args.output/'vip-membership-selected.png')
assert len(m.sent)==before
pix,w,h=m.pixels(q)
left=m.r(pix+(28*w+80)*4);right=m.r(pix+(60*w+80)*4)
...
print('PASS: five pale-blue beveled cards on approved icy popup, transparent crown, vertical cream/gold selection, regular day labels, right-aligned server prices through 1,000,000,000z; selection never pays.')
